...
export const CompareWindow: React.FC<CompareWindowProps> = ({
  ...
  return (
    <div className="w-full h-screen relative" style={{ background: 'transparent' }}>
      ...
      {/* 浮动控制面板 */}
      {isControlsVisible && (
        <div className="absolute top-4 right-4 bg-black bg-opacity-90 text-white rounded-xl p-4 backdrop-blur-sm z-50 min-w-64">
      ...
      {!isControlsVisible && (
        <button
          ...
          className="absolute top-4 right-4 bg-black bg-opacity-70 text-white rounded-xl px-3 py-2 text-sm font-medium hover:bg-opacity-90 z-50 transition-all"
          title="显示控制面板 (空格)"
        >
          ⚙ 控制
        </button>
